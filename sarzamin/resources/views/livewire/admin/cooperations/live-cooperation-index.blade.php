                ...
                <table class="table">
                    <thead>
                        <tr>
                            ...
                            <th scope="col">{{ __('global.desired_job') }}</th>
                    ...
                    <tbody>
                        @foreach ($cooperations as $key => $cooperation)
                            ...
                            <tr>
                                <th scope="row">{{  ($cooperations->currentpage()-1) * $cooperations->perpage() + $key + 1 }}</th>
                                <td wire:click="show({{ $cooperation->id }})" class="cursor-pointer text-nowrap">{{ $cooperation->name ?: "_" }}</td>
                                <td wire:click="show({{ $cooperation->id }})" class="cursor-pointer text-nowrap" dir="ltr">{{ $cooperation->phone }}</td>
                                <td wire:click="show({{ $cooperation->id }})" class="cursor-pointer text-nowrap">{{  $cooperation->job }}</td>
                                <td class="text-nowrap">{{ \Morilog\Jalali\Jalalian::fromDateTime($cooperation->created_at)->format('Y-m-d') }}</td>
                                <td class="text-nowrap">
                                    <i class="cursor-pointer ti ti-trash text-danger ms-2" data-bs-toggle="tooltip" data-bs-placement="top" onclick="Custom.deleteItemList({{$cooperation->id}})" title="حذف"></i>
                                    {{-- <i class="cursor-pointer ti ti-pencil text-warning ms-2" data-bs-toggle="tooltip" data-bs-placement="top" wire:click="edit({{ $cooperation->id }})" title="ویرایش"></i> --}}
                                    <i class="cursor-pointer ti ti-eye" data-bs-toggle="tooltip" data-bs-placement="top" wire:click="show({{ $cooperation->id }})" title="نمایش"></i>
                                </td>
                            </tr>
                        @endforeach
                    </tbody>
                </table>
                {{ $cooperations->links("pagination::bootstrap-5") }}
            </div>
        </div>
    </div>
</div>
