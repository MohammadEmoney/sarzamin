    ...
    <div class="card">
        <div class="card-body">
            <div class="d-flex justify-content-between mb-2">
                ...
                <button class="btn btn-sm btn-info" wire:click="create">{{ __('global.create_comment') }}</button>
            </div>
                ...
                <table class="table">
                    <thead>
                        <tr>
                            ...
                            <th scope="col">{{ __('global.comment') }}</th>
                            <th scope="col">{{ __('global.news') }}</th>
                    ...
                    <tbody>
                        @foreach ($comments as $key => $comment)
                            ...
                            <tr>
                                <th scope="row">{{  ($comments->currentpage()-1) * $comments->perpage() + $key + 1 }}</th>
                                <td wire:click="show({{ $comment->id }})" class="cursor-pointer text-nowrap">{{ $comment->title ?: "_" }}</td>
                                <td wire:click="show({{ $comment->id }})" class="cursor-pointer text-nowrap text-truncate" style="max-width: 150px;">{{ $comment->message }}</td>
                                <td wire:click="show({{ $comment->commentable?->id }})" class="cursor-pointer text-nowrap">{{ $comment->commentable?->title ?: "-" }}</td>
                                
                                <td class="text-nowrap">{{ \Morilog\Jalali\Jalalian::fromDateTime($comment->created_at)->format('Y-m-d') }}</td>
                                <td class="text-nowrap">
                                    <select class="form-control" wire:model="status" id="">
                                        @foreach ($statuses as $key => $value)
                                            <option value="{{ $key }}" @selected($comment->status === $key)>{{ $value }}</option>
                                        @endforeach
                                    </select>
                                </td>
                                <td class="text-nowrap">
                                    <i class="cursor-pointer ti ti-trash text-danger ms-2" data-bs-toggle="tooltip" data-bs-placement="top" onclick="Custom.deleteItemList({{$comment->id}})" title="حذف"></i>
                                    <i class="cursor-pointer ti ti-eye" data-bs-toggle="tooltip" data-bs-placement="top" wire:click="show({{ $comment->id }})" title="نمایش"></i>
                                </td>
                            </tr>
                        @endforeach
                    </tbody>
