                    ...
                    <tbody>
                        @foreach ($contacts as $key => $contact)
                            ...
                            <tr>
                                ...
                                <td wire:click="show({{ $contact->id }})" class="cursor-pointer text-nowrap">{{ $contact->name ?: "_" }}</td>
                                <td wire:click="show({{ $contact->id }})" class="cursor-pointer text-nowrap" dir="ltr">{{ $contact->phone }}</td>
                                <td wire:click="show({{ $contact->id }})" class="cursor-pointer text-nowrap">{{ $contact->subject }}</td>
                                <td class="text-nowrap">{{ \Morilog\Jalali\Jalalian::fromDateTime($contact->created_at)->format('Y-m-d') }}</td>
                                <td class="text-nowrap">
                                    <i class="cursor-pointer ti ti-trash text-danger ms-2" data-bs-toggle="tooltip" data-bs-placement="top" onclick="Custom.deleteItemList({{$contact->id}})" title="حذف"></i>
                                    {{-- <i class="cursor-pointer ti ti-pencil text-warning ms-2" data-bs-toggle="tooltip" data-bs-placement="top" wire:click="edit({{ $contact->id }})" title="ویرایش"></i> --}}
                                    <i class="cursor-pointer ti ti-eye" data-bs-toggle="tooltip" data-bs-placement="top" wire:click="show({{ $contact->id }})" title="نمایش"></i>
                                </td>
                            </tr>
                        @endforeach
                    </tbody>
